...
import { deleteJob } from '../actions/jobTitleCreateAction';
 ...
 const MainJobScreen = () => {
    ...
    let [updatedJobTitles,setUpdatedJob]=useState([])
    const dispatch = useDispatch();
    const [_id,setId]=useState('')
    const jobTitleList = useSelector((state) => state.jobtitlelist);
    const {jobTitles, loading, error } = jobTitleList 
    // const [isShow,setTogleShow]=useState(true)
    const jobTitleDelete = useSelector((state) => state.deletejobtitle);
    const {loading:deleteLoading, error:deleteError,success,failError } = jobTitleDelete 
 ...
 //  const pageNo=useMemo(()=>NumberOfPages(itemsPerPage),[data])
       ...
       await dispatch(deleteJob(_id))
        ...
        const searchHandler = (value) => {
          setSearchValue(value);
            if(value)
                {
                  const fiteredData= jobTitles.filter((job) => {
        ...
        console.log('number of pages ',Math.ceil(updatedJobTitles.length / itemsPerPage))
        ...
        updatedJobTitles = updatedJobTitles.slice(indexOfFirstItem, indexOfLastItem);
 ...
 //end of utilites for Pagination
  ...
  return (
    ...
    <Modal  show={show} onHide={handleClose}>
      ...
      <div className="mb-4 shadow-sm">
        <header className="bg-white">
          <div className="row gx-3 py-3">
            ...
            <Link to={`/jobtitle`} className="text-add">
      ...
      <tbody>
      {
        updatedJobTitles.map((jobTitle, index) => {
                    return (
                      <tr key={jobTitle._id}>
                        <td>{index+1}</td>
                        <td>{jobTitle.code}</td>
                        ...
                        <td className="d-flex justify-content-end align-item-center">
                          <Link to={`/editjobtitle/${jobTitle._id}`} className="text-success">
                            <i className="fas fa-pencil"></i>&nbsp;&nbsp;
                          </Link>
                          <Link to onClick={()=>deleteHandler(jobTitle._id)} className="text-danger">
   ...
   <li>
                ...
                <span></span>
                { searchValue ? ( <span>&nbsp;From  {Math.ceil(updatedJobTitles.length / itemsPerPage)} </span>):
